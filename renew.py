            ...
            if remaining_days <= 2:
                ...
                if sb.is_element_visible(renew_btn_selector):
                    ...
                    try:
                        ...
                        # 截图：目标确认图
                        sb.driver.save_screenshot(str(screenshot_target))
                        tg_send_photo(str(screenshot_target), "🔍 【排查步骤】已锁定续期按钮（红框标出）")
                        
                        sb.click(renew_btn_selector)
                        print("点击动作已执行，等待后端处理...")
                    except Exception as e:
                        print(f"标红或点击异常: {e}")

                    # ── 🔥 核心优化：等待几秒后重新刷新并读取日期进行比对 ──
                    time.sleep(6)
                    
                    new_date_str = ""
                    try:
                        print("正在重新加载服务页以检查最新到期时间...")
                        sb.driver.get(SERVICES_URL)
                        time.sleep(4)
                        ...
                        print(f"[WARN] 重新加载服务页异常: {reload_err}")
                    ...
                    if new_date_str and new_date_str != old_date_str:
                        ...
                        msg = (
                            ...
                            f"📅 旧到期日: {old_date_str}\n"
                            f"📅 当前到期日: {new_date_str or '获取失败'}\n"
                            f"💬 提示: 点击后重新检查，到期时间未发生变化。"
                        )
                        print(msg)
                        try:
                            tg_send_photo(str(screenshot_final), msg)
